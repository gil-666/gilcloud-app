...
<template>
  <div ref="folderItemRef" @click.right.prevent="showDropdown = !showDropdown"
    class="folder-item relative rounded-2xl p-2 border-1 border-neutral-700 w-full h-full ">
    <div @click.stop="showDropdown = !showDropdown;" class="pi pi-ellipsis-h p-1 menu rounded-2xl absolute z-10"></div>
    <div class="dropdown-menu absolute z-100 mt-5"
      :class="showDropdown ? 'translate-y-2 opacity-100 pointer-events-auto' : '-translate-y-5 opacity-0 pointer-events-none'">
      <ul class="bg-neutral-700 border-1 border-zinc-500">
        ...
        <li @click.stop="emit('link-generate-view'); showDropdown = false" class="p-2 dropdown-btn">Share link</li>
        <!-- <li @click.stop="emit('link-generate');showDropdown = false" class="p-2 dropdown-btn">Share download link</li> -->
